...
## 最新更新 (v1.70)
* **唯讀類工具預設免授權**: 讀網頁、列出/檢視圖片、YouTube 字幕、開新分頁、切換分頁不再逐次詢問，閱讀與查核流程不會被打斷。`execute_javascript_on_page`（能改動網頁、發出請求）與 `see_current_screen`（可能截到與任務無關的內容）維持人工確認。
* **開新分頁強制顯示目標網址**: 開分頁雖然自動放行，但動作紀錄會固定顯示代理要前往的完整網址，不折疊隱藏——網址是資料可能被夾帶送出的出口，值得你一眼看見。
* 一批工具中只要含有需確認的工具，整批都會回到人工確認。
...
## 開發:
工具授權邏輯（哪些工具免詢問、哪些必須人工確認）有一份回歸測試，需要 Node.js：

```
node test/grant-logic.mjs
```

改動 `sidepanel.js` 中的 `AUTO_GRANTED_TOOLS` 或授權判斷後建議跑一次。測試檔不在 `release.ps1` 的打包清單中，不會進入擴充功能。
